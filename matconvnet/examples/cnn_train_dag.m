...
K = length(imdb);
opts.numEpochs = opts.numCycles * K ;
...
%   params.train = opts.train(randperm(numel(opts.train))) ; % shuffle
  seq_id = mod(seq_id-1, K)+1;
  if seq_id == 1
      loss = 0;
  end;
  train_idx_start = floor((epoch-1) / K)*opts.batch_frames + 1;
  train_idx_end = floor((epoch-1) / K)*opts.batch_frames + opts.batch_frames;
  
%   params.train = opts.train{mod(seq_id, K)}();
  %params.val = opts.val(randperm(numel(opts.val))) ;
  params.train = opts.train{mod(seq_id-1, K)+1}(train_idx_start:train_idx_end);
%   fprintf('%d: %d %d: ', seq_id, train_idx_start, train_idx_end);
fprintf('.');
  
  params.imdb = imdb{mod(seq_id-1, K)+1} ;
...
start = tic ;
for t=1:params.batch_frames:numel(subset)
%   fprintf('%s: epoch %02d: %3d/%3d:', mode, epoch, ...
%           fix((t-1)/params.batchSize)+1, ceil(numel(subset)/params.batch_frames)) ;
  batch_frames = min(params.batch_frames, numel(subset) - t + 1) ;
  batchSize = params.batchSize;

  for s=1:params.numSubBatches
    % get this image batch and prefetch the next
    batchStart = t + (labindex-1) + (s-1) * numlabs ;
    batchEnd = min(t+params.batch_frames-1, numel(subset)) ;
    batch = subset(batchStart : params.numSubBatches * numlabs : batchEnd) ;
    num = num + numel(batch) ;
    if numel(batch) == 0, continue ; end

    inputs = params.getBatch(params.imdb, batch, ...
                             params.batch_pos, params.batch_neg, ...
