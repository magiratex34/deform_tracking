  ...
  try
    for l = 1:numel(s.layers)
      ...
      block = constr() ;
       ...
       %------------yb added-------
%       if strcmp(s.layers(l).type,'dagnn.Conv')||...
%               strcmp(s.layers(l).type,'dagnn.Pooling')
%           s.layers(l).block=rmfield(s.layers(l).block,'dilate');
%       end
%       
%       if strcmp(s.layers(l).type,'dagnn.BatchNorm')
%           s.layers(l).block=rmfield(s.layers(l).block,'opts');
%       end
      %------------end------------          
