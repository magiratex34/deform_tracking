function [ result ] = run_deformnet(images, region, net, display)
% 
...
[net_conv, net_fc, opts] = init_model(img, net, 'dagnn');
...
if(opts.bbreg)
    ...
    feat_conv = extract_deepfeat(net_conv, img, pos_examples, opts);
...
feat_conv = extract_deepfeat(net_conv, img, examples, opts);
...
net_fc = online_train(net_fc,pos_data,neg_data,opts,...
...
for To = 2:nFrames
    ...
    feat_conv = extract_deepfeat(net_conv, img, samples, opts);
    
    % evaluate the candidates
    feat_fc = classifer(net_fc, feat_conv, opts);
    ...
    if((mod(To,opts.update_interval)==0 || target_score<0) && To~=nFrames)
        ...
        net_fc = reset_layer(net_fc, 'st_fc_outf');
...
function net = reset_layer(net, name)
f = net.getParamIndex(name);
fsz = size(net.params(f).value);
net.params(f).value = gpuArray(zeros(fsz, 'single'));
net.params(f).momentum = gpuArray(zeros(fsz, 'single'));
