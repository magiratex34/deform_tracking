function [ bb_samples ] = gen_samples_sub(type, bb, n, opts, trans_f, scale_f)
% GEN_SAMPLES
...
switch (type)
    ...
    case 'xwindow' % search for valid sample
        xx = 4;
        lbd = max([bb(3)/2,      bb(1)-xx*bb(3)]);
        rbd = min([w-bb(3)/2,    bb(1)+xx*bb(3)]);
        tbd = max([bb(4)/2,      bb(2)-xx*bb(4)]);
        bbd = min([h-bb(4)/2,    bb(2)+xx*bb(4)]);
        
        range = round([lbd, tbd, rbd, bbd]);
        stride = round([bb(3)/2 bb(4)/2]);
        [dx, dy, ds] = meshgrid(range(1):stride(1):range(3), range(2):stride(2):range(4), -2:2);
        
        windows = [dx(:) dy(:) bb(3)+ones(numel(dx),1) bb(4)+ones(numel(dx),1)];
